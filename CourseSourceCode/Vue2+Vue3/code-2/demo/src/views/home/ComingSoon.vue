      ...
      <van-cell v-for="data in datalist" :key="data.filmId"
        ...
        <div class="listItem" @click="handleClick(data.filmId)">
          ...
          <div class="textInfo">
            ...
            <p>上映日期：{{ formatterDate(data.premiereAt) }}</p>
          </div>
          <button v-if="data.isPresale">预购</button>
        </div>
      </van-cell>
    </van-list>
  </div>
...
export default {
  ...
  methods: {
    ...
    formatterDate (timestamp) {
      // 设置中文语言环境
      moment.locale('zh-cn')
      return moment(timestamp * 1000).format('周dd MMMDo')
    },
    onLoad () {
      ...
      http({
        url: `/gateway?cityId=${this.$store.state.cityId}&pageNum=${this.pageNum++}&pageSize=10&type=2&k=5981303`,
        headers: {
